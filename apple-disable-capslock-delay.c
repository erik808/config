//
//  This can be used to remove the built-in capslock delay on some apple keyboards
//
//  Compilation: gcc -o apple-disable-capslock-delay apple-disable-capslock-delay.c
//
//  Usage:
//    HIDDEVICE=$(dmesg | grep Apple | grep Keyboard | grep input0 | tail -1 | sed -e 's/.*hidraw\([[:digit:]]\+\).*/\/dev\/hidraw\1/')
//  ./apple-disable-capslock-delay $HIDDEVICE
//
