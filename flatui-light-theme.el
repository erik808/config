...
(deftheme flatui-light "FlatUI based color theme.")

;;; Color Palette

(defvar flatui-light-colors-alist
  '(("clouds"          . "#FBFCFC")
    ("silver"          . "#FBFCFC")
    ("concrete"        . "#546363")
    ("asbestos"        . "#7f8c8d")
    ("wet-asphalt"     . "#243341")
    ("midnight-blue"   . "#111820")
...
(defmacro flatui-light/with-color-variables (&rest body)
  "`let' bind all colors defined in `flatui-light-colors-alist' around BODY.
Also bind `class' to ((class color) (min-colors 89))."
  (declare (indent 0))
  `(let ((class '((class color) (min-colors 89)))
         ,@(mapcar (lambda (cons)
                     (list (intern (car cons)) (cdr cons)))
                   flatui-light-colors-alist))
     ,@body))

;;; Theme Faces
(flatui-light/with-color-variables
...
(provide-theme 'flatui-light)

;; Local Variables:
;; no-byte-compile: t
;; indent-tabs-mode: nil
;; End:
;;; flatui-light-theme.el ends here
